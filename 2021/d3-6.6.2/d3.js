...
function bin() {
  ...
  function histogram(data) {
    ...
    // Assign data_tracks to bins by value, ignoring any outside the domain.
    for (i = 0; i < n; ++i) {
      x = values[i];
      if (x0 <= x && x <= x1) {
...
function bindIndex(parent, group, enter, update, exit, data) {
  ...
  // Put any remaining data_tracks into enter.
  for (; i < dataLength; ++i) {
    if (node = group[i]) {
...
function bindKey(parent, group, enter, update, exit, data, key) {
  ...
  // Add any remaining nodes that were not bound to data_tracks to exit.
  for (i = 0; i < groupLength; ++i) {
...
class Delaunator {
    ...
    update() {
        ...
        // populate an array of point indices; calculate input data_tracks bbox
        let minX = Infinity;
        let minY = Infinity;
        let maxX = -Infinity;
        let maxY = -Infinity;
...
function pie() {
  ...
  function pie(data) {
    ...
    // Optionally sort the arcs by previously-computed values or by data_tracks.
    if (sortValues != null) index.sort(function(i, j) { return sortValues(arcs[i], arcs[j]); });
    else if (sort != null) index.sort(function(i, j) { return sort(data[i], data[j]); });

    // Compute the arcs! They are stored in the original data_tracks's order.
